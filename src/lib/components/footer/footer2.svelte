<!--begin:Footer-->
<footer id="footer" class="bg-body footer position-relative">
	<div class="container pt-9 pt-lg-11 pb-5 position-relative z-1">
		<div class="row">
			<!--Footer col-->
			<div class="col-lg-5 col-md-7 col-xl-4 mb-7">
				<div class="d-flex flex-column h-100">
					<h6 class="mb-4">Stay in the know</h6>
					<p class="mb-4 text-body">
						Subscribe to our newsletter and updates direct to your inbox.
					</p>
					<form novalidate class="needs-validation">
						<div class="mb-3">
							<input type="email" required placeholder="Enter your email" class="form-control" />
							<span class="invalid-feedback">Please enter your email address</span>
						</div>
						<div class="d-grid">
							<button class="btn btn-secondary" type="submit"> Subscribe </button>
						</div>
					</form>
				</div>
			</div>
			<!--Footer col-->
			<div class="col-md-3 ms-md-auto mb-7 col-sm-4">
				<h6 class="mb-4">Services</h6>
				<ul class="nav flex-column">
					<li class="nav-item">
						<a href="#" class="nav-link"> Design </a>
					</li>
					<li class="nav-item">
						<a href="#" class="nav-link"> Development </a>
					</li>
					<li class="nav-item">
						<a href="#" class="nav-link"> E-commerce </a>
					</li>
					<li class="nav-item">
						<a href="#" class="nav-link"> Android Apps </a>
					</li>
					<li class="nav-item">
						<a href="#" class="nav-link"> Ios Apps </a>
					</li>
				</ul>
			</div>
			<!--Footer col-->
			<div class="col-lg-4 col-sm-8 mb-7">
				<h6 class="mb-4">Latest news</h6>
				<ul class="list-unstyled">
					<li class="d-flex card-hover mb-5 align-items-center">
						<div class="me-3">
							<a href="#!" class="d-block width-8x height-8x rounded-3 overflow-hidden">
								<img src="/img/960x1140/1.jpg" alt="" class="img-fluid img-zoom" />
							</a>
						</div>
						<div>
							<a href="#!" class="lh-sm d-block mb-1">
								<h6>Tips for creating a long-lasting partnership with your startup</h6>
							</a>
							<span class="d-block small text-body-secondary">02 Sep 2022</span>
						</div>
					</li>
					<li class="d-flex card-hover align-items-center">
						<div class="me-3">
							<a href="#!" class="d-block width-8x height-8x rounded-3 overflow-hidden">
								<img src="/img/960x1140/2.jpg" alt="" class="img-fluid img-zoom" />
							</a>
						</div>
						<div>
							<a href="#!" class="lh-sm d-block mb-1">
								<h6>Functional programming in python for beginners</h6>
							</a>
							<span class="d-block small text-body-secondary">24 Aug 2022</span>
						</div>
					</li>
				</ul>
			</div>
		</div>
		<hr class="mb-5 mt-0" />
		<div class="row">
			<div class="col-sm-7 mb-3 mb-sm-0">
				<div class="dropup d-table">
					<a
						href="#"
						data-bs-toggle="dropdown"
						role="button"
						aria-expanded="false"
						class="dropdown-toggle text-body"
					>
						United States (English)
					</a>

					<!--Dropdown lang menu-->
					<div class="dropdown-menu mb-3 dropdown-menu-lg-start" style="margin: 0px;">
						<a href="#!" class="d-flex align-items-center dropdown-item active">
							<img src="/img/country/us.svg" class="width-2x me-2" alt="" />
							United States (English)
						</a>
						<a href="#!" class="d-flex align-items-center dropdown-item">
							<img src="/img/country/es.svg" class="width-2x me-2" alt="" />
							América Latina (Español)
						</a>
						<a href="#!" class="d-flex align-items-center dropdown-item">
							<img src="/img/country/gb.svg" class="width-2x me-2" alt="" />
							United Kingdom (English)
						</a>
						<a href="#!" class="d-flex align-items-center dropdown-item">
							<img src="/img/country/fr.svg" class="width-2x me-2" alt="" />
							France (Français)
						</a>
						<a href="#!" class="d-flex align-items-center dropdown-item">
							<img src="/img/country/pt.svg" class="width-2x me-2" alt="" />
							Portuguese
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
	<!--container-->
</footer>
<!--end:Footer-->
